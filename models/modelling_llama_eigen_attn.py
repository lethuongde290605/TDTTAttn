...
class LlamaEigenAttention(nn.Module):
    ...
    def __init__(self, rank_kq, rank_v,  config: LlamaConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.q_proj = torch.nn.Linear(self.hidden_size, self.hidden_size, bias = config.attention_bias)

        self.v_proj_low = torch.nn.Linear(self.hidden_size, self.rank_v, bias = config.attention_bias)

        self.o_proj_up = nn.Linear(self.num_heads * (self.rank_v // self.num_key_value_heads), self.hidden_size, bias=config.attention_bias)
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if "rope_type" in self.config.rope_scaling:
                self.rotary_emb = LlamaRotaryEmbedding(
                    dim=self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
    ...
    def forward(
        ...
        **kwargs,
        ...
        #note : we save KV cache before applying rope embedding in  its low dimensional form. The low dim KV are projected back and Rope is applied online
        cos, sin = self.rotary_emb(value_states_low, position_ids)
        past_key_value = getattr(self, "past_key_value", past_key_value)
        if past_key_value is not None:
            # sin and cos are specific to RoPE models; cache_position needed for the static cache
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
            key_states_low, value_states_low = past_key_value.update(key_states_low, value_states_low, self.layer_idx, cache_kwargs)

        # value_states = self.v_proj_up(value_states_low)
...
class LlamaEigenAttnDecoderLayer(nn.Module):
    def __init__(self, low_rank_config, config: LlamaConfig, layer_idx: int):
        ...
        if config._attn_implementation != "eager":
            import warnings
            warnings.warn("attention implementaion was set to " + config._attn_implementation +  ", changing it to eager!!")
            config._attn_implementation = "eager"
        assert config._attn_implementation == "eager" #only supprt eager attention. ToDo : Add support for Flash Attention and SDPA attention
        self.self_attn = LlamaEigenAttention(rank_kq = int(low_rank_config[0]), rank_v = int(low_rank_config[1]), config=config, layer_idx=layer_idx)
...
class LlamaModel_EigenAttn(LlamaPreTrainedModel):
    """
    ...
    def __init__(self, low_rank_config, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaEigenAttnDecoderLayer(low_rank_config=low_rank_config[layer_idx], config = config, layer_idx = layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LlamaForCausalLM_EigenAttn(LlamaPreTrainedModel):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.model = LlamaModel_EigenAttn(low_rank_config=low_rank_config, config=config)
        self.vocab_size = config.vocab_size
...
class LlamaForSequenceClassification_EigenAttn(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = LlamaModel_EigenAttn(config)
...
class LlamaForQuestionAnswering_EigenAttn(LlamaPreTrainedModel):
    base_model_prefix = "transformer"

    # Copied from transformers.models.bloom.modeling_bloom.BloomForQuestionAnswering.__init__ with Bloom->Llama
    def __init__(self, config):
        super().__init__(config)
        self.transformer = LlamaModel_EigenAttn(config)
